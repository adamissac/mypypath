...
(function () {
  ...
  /* The wording is load-bearing, and it now matches what happens.
   *
   * For a while this said an account "opens the rest" while a second lock
   * behind it told the new account holder to go and finish the unit before --
   * two gates, one promise, and the promise was the one they had just acted
   * on. An account does open every unit now. What stays in order is credit: a
   * unit is completed once the one before it is finished, which is the
   * sentence below. */
  function paywallHtml(unit) {
    ...
    return '<div class="gate-paywall" data-gate-paywall>' +
      ...
      '<p>Units 1 and ' + FREE_UNITS + ' are free to everyone. An account opens every ' +
      'unit to read and work through, and keeps your progress on every device. ' +
      'Units still count in order: each one is credited once you finish the one ' +
      'before it.</p>' +

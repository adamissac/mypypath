...
/* The AI grading cap and its cost log. Neither is written by the grading
   function: api/grade.js holds no Firebase credentials at all, so everything
   here is written by the client under the client's own token and constrained
   only by these rules, which is the same arrangement the whole app runs on. */

describe('the AI grading cap', () => {
  const path = (uid) => `users/${uid}/state/aiGrading`;
  const today = new Date().toISOString().slice(0, 10);

  it('lets a learner start today at any count', async () => {
    await assertSucceeds(
      setDoc(doc(as('ann'), path('ann')), { day: today, count: 1, updatedAt: Date.now() })
    );
  });

  it('lets the count go up by exactly one', async () => {
    await env.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), path('ann')), { day: today, count: 5, updatedAt: 1 });
    });
    await assertSucceeds(
      setDoc(doc(as('ann'), path('ann')), { day: today, count: 6, updatedAt: Date.now() })
    );
  });

  /* The two ways a client would cheat if the rule only checked the shape. */
  it('denies winding the count back', async () => {
    await env.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), path('ann')), { day: today, count: 5, updatedAt: 1 });
    });
    await assertFails(
      setDoc(doc(as('ann'), path('ann')), { day: today, count: 0, updatedAt: Date.now() })
    );
    await assertFails(
      setDoc(doc(as('ann'), path('ann')), { day: today, count: 7, updatedAt: Date.now() })
    );
  });

  it('denies smuggling an extra key into the counter', async () => {
    await assertFails(
      setDoc(doc(as('ann'), path('ann')),
        { day: today, count: 1, updatedAt: Date.now(), unlimited: true })
    );
  });

  it('denies writing somebody else\'s counter', async () => {
    await assertFails(
      setDoc(doc(as('mallory'), path('ann')), { day: today, count: 1, updatedAt: Date.now() })
    );
  });

  it('leaves every other state document writable as before', async () => {
    await assertSucceeds(
      setDoc(doc(as('ann'), 'users/ann/state/prefs'), { anything: 'at all' })
    );
  });
});

describe('the AI cost log', () => {
  const day = 'aiUsage/2026-08-27';

  it('lets a signed-in learner append to the day', async () => {
    await assertSucceeds(
      setDoc(doc(as('ann'), day), { calls: 1, inputTokens: 900, outputTokens: 120, updatedAt: 1 })
    );
  });

  it('denies writing a smaller total over a larger one', async () => {
    await env.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), day),
        { calls: 40, inputTokens: 1, outputTokens: 1, updatedAt: 1 });
    });
    await assertFails(
      setDoc(doc(as('ann'), day), { calls: 2, inputTokens: 1, outputTokens: 1, updatedAt: 2 })
    );
  });

  /* Volume and cost, never who needed the most help. A teacher has no read
     here at all, and neither does anybody but staff. */
  it('denies reading it to a learner and to a teacher', async () => {
    await assertFails(getDoc(doc(as('ann'), day)));
    await assertFails(getDoc(doc(as('teacherA'), day)));
  });

  it('denies deleting a day, so a bill cannot be tidied away', async () => {
    await assertFails(deleteDoc(doc(as('ann'), day)));
  });
});
